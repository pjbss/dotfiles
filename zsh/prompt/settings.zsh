...
PROMPT='%{$fg_bold[magenta]%}%n%{$reset_color%}@%{$fg_bold[cyan]%}$(box_name)%{$reset_color%}:%{$fg_bold[green]%}%~%{$reset_color%}$(git_prompt_info)$(git_prompt_worktree_count)$(git_prompt_ahead_behind)$(python_prompt_info)$(aws_prompt_info) $ '
